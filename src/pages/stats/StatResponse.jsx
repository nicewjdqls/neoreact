...
function StatResponse() {
  ...
  const validateFilters = () => {
    if (filters.startDate && filters.endDate) {
      ...
      // 조회 기간이 너무 긴지 체크 (예: 14일 이상)
      const daysDiff = (endDateTime - startDateTime) / (1000 * 60 * 60 * 24);
      if (daysDiff > 14) {
        showNotification(
          '조회 기간 제한',
          '조회 기간은 14일을 초과할 수 없습니다. 기간을 줄여주세요.',
          'warning'
        );
        return false;
      }
  ...
  const applyFilters = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        showNotification(
          '조회 완료',
          `총 ${filtered.length}건의 응답시간 데이터가 조회되었습니다.`,
          'success'
        );
      }, 100);
  ...
  const getResponseTimeClass = (ms) => {
    ...
    if (time < 100) return 'text-green-600 font-semibold';
    if (time < 300) return 'text-yellow-600 font-semibold';
    ...
    >
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
          <h2 className="text-xl font-bold text-gray-900">평균 응답시간 상세조회</h2>
        </div>

        {/* 필터 영역 */}
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
          {/* 첫 번째 행: 시작일시 - 종료일시 - 서비스구분 - 조회버튼 */}
        ...
        <div className="flex justify-between items-center">
          <span className="text-sm text-gray-600">
            총 {filteredResponses.length}건의 응답시간 데이터가 조회되었습니다.
          </span>
          {autoScroll && (
              ...
              <table className="w-full">
                <thead className="bg-gray-50 sticky top-0 z-10">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-center text-sm font-semibold text-gray-600 border-b border-gray-200">요청 수</th>
                    <th className="px-4 py-3 text-center text-sm font-semibold text-gray-600 border-b border-gray-200">평균 응답시간(ms)</th>
                    <th className="px-4 py-3 text-center text-sm font-semibold text-gray-600 border-b border-gray-200">모델 지연(ms)</th>
                    <th className="px-4 py-3 text-center text-sm font-semibold text-gray-600 border-b border-gray-200">네트워크 지연(ms)</th>
                    <th className="px-4 py-3 text-center text-sm font-semibold text-gray-600 border-b border-gray-200">총 응답시간(ms)</th>
                    <th className="px-4 py-3 text-center text-sm font-semibold text-gray-600 border-b border-gray-200">최대(ms)</th>
                    <th className="px-4 py-3 text-center text-sm font-semibold text-gray-600 border-b border-gray-200">최소(ms)</th>
                    <th className="px-4 py-3 text-center text-sm font-semibold text-gray-600 border-b border-gray-200">마지막 요청 시간</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-200">
                  {filteredResponses.map(response => (
                    <tr key={response.id} className="hover:bg-gray-50">
                      <td className="px-4 py-2 text-center text-sm font-mono text-gray-900">{response.sessionId}</td>
                      <td className="px-4 py-2 text-center text-sm font-medium text-gray-900">{response.service}</td>
                      <td className="px-4 py-2 text-center text-sm font-semibold text-blue-600">{response.count}</td>
                      <td className={`px-4 py-2 text-center text-sm ${getResponseTimeClass(response.avg)}`}>
                        {response.avg}
                      </td>
                      <td className="px-4 py-2 text-center text-sm text-gray-700">{response.modelMs}</td>
                      <td className="px-4 py-2 text-center text-sm text-gray-700">{response.networkMs}</td>
                      <td className={`px-4 py-2 text-center text-sm font-semibold ${getResponseTimeClass(response.totalMs)}`}>
                        {response.totalMs}
                      </td>
                      <td className="px-4 py-2 text-center text-sm text-red-600 font-medium">{response.max}</td>
                      <td className="px-4 py-2 text-center text-sm text-green-600 font-medium">{response.min}</td>
                      <td className="px-4 py-2 text-center text-sm text-gray-700">{formatDateTime(response.lastTime)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>

...
function StatResponse() {
  ...
  const [filters, setFilters] = useState({ startDate: '', endDate: '', serviceType: '' });
  ...
  const addResponse = () => {
    const now = new Date();
    const sessionId = 'sess-' + Math.floor(Math.random() * 100);
    const service = services[Math.floor(Math.random() * services.length)];
    const count = Math.floor(Math.random() * 50) + 1;
    const modelMs = (Math.random() * 300 + 50).toFixed(2);
    const networkMs = (Math.random() * 100 + 20).toFixed(2);
    const totalMs = (Number(modelMs) + Number(networkMs)).toFixed(2);
    const avg = ((Number(modelMs) + Number(networkMs)) / 2).toFixed(2);
    const max = (Number(totalMs) + Math.floor(Math.random() * 200)).toFixed(2);
    const min = Math.max(0, Number(totalMs) - Math.floor(Math.random() * 50)).toFixed(2);

    const newResponse = { id: uniqId(), sessionId, service, count, avg, modelMs, networkMs, totalMs, max, min, lastTime: now, rawTime: now };
    setResponses(prev => {
  ...
  const applyFilters = async () => {
    ...
    try {
      ...
      if (filters.startDate) {
        ...
        filtered = filtered.filter(response => response.rawTime >= startTime);
      }
      if (filters.endDate) {
        const endTime = new Date(filters.endDate);
        const endOfDay = new Date(endTime);
        endOfDay.setHours(23, 59, 59, 999);
        filtered = filtered.filter(response => response.rawTime <= endOfDay);
      }
      if (filters.serviceType) {
        filtered = filtered.filter(response => response.service === filters.serviceType);
      }
      filtered.sort((a, b) => b.rawTime - a.rawTime);
      setFilteredResponses(filtered);
      setTimeout(() => {
        showNotification('조회 완료', `총 ${filtered.length}건의 응답시간 데이터가 조회되었습니다.`, 'success');
      }, 100);
    } catch (error) {
  ...
  const getResponseTimeClass = (ms) => {
    ...
    if (time < 100) return { color: 'rgba(134, 239, 172, 1)', fontWeight: '600' };
    if (time < 300) return { color: 'rgba(252, 211, 77, 1)', fontWeight: '600' };
  ...
  useEffect(() => {
    const interval = setInterval(addResponse, 1500);
    return () => clearInterval(interval);
  }, []);

  useEffect(() => {
    if (autoScroll && responses.length > 0) {
      ...
      <div style={{ padding: '1.5rem' }}>
        <div style={{ marginBottom: '1.5rem' }}>
          <div style={{ background: 'rgba(42, 48, 70, 0.6)', border: '1px solid rgba(139, 92, 246, 0.3)', borderRadius: '12px', padding: '1.5rem' }}>
            <h2 style={{ fontSize: '1.25rem', fontWeight: '700', color: 'rgba(255, 255, 255, 0.95)', marginBottom: '0' }}>평균 응답시간 상세조회</h2>
          </div>
        </div>
        ...
        <div style={{ background: 'rgba(42, 48, 70, 0.6)', border: '1px solid rgba(139, 92, 246, 0.3)', borderRadius: '12px', padding: '1.5rem', marginBottom: '1.5rem' }}>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end mb-4">
            ...
            <div className="md:col-span-1">
              ...
              <select value={filters.serviceType} onChange={(e) => handleFilterChange('serviceType', e.target.value)} className="w-full px-3 py-2 text-sm border rounded-lg transition-all duration-200" style={{ background: 'rgba(42, 48, 70, 0.6)', borderColor: 'rgba(139, 92, 246, 0.3)', color: 'rgba(255, 255, 255, 0.9)' }}>
        ...
        <div className="flex justify-between items-center" style={{ marginBottom: '1.5rem' }}>
          <span className="text-sm" style={{ color: 'rgba(255, 255, 255, 0.8)' }}>총 {filteredResponses.length}건의 응답시간 데이터가 조회되었습니다.</span>
          {autoScroll && (
            ...
            {filteredResponses.length > 0 ? (
              <table className="w-full">
                <thead className="sticky top-0 z-10" style={{ background: 'rgba(30, 33, 57, 0.95)' }}>
                  <tr>
                    {['세션ID', '서비스 구분', '요청 수', '평균 응답시간(ms)', '모델 지연(ms)', '네트워크 지연(ms)', '총 응답시간(ms)', '최대(ms)', '최소(ms)', '마지막 요청 시간'].map(header => (
                ...
                <tbody>
                  {filteredResponses.map(response => {
                    const avgStyle = getResponseTimeClass(response.avg);
                    const totalStyle = getResponseTimeClass(response.totalMs);
                    return (
                      <tr key={response.id} className="border-b transition-colors" style={{ borderColor: 'rgba(139, 92, 246, 0.1)' }} onMouseEnter={(e) => e.currentTarget.style.background = 'rgba(139, 92, 246, 0.05)'} onMouseLeave={(e) => e.currentTarget.style.background = 'transparent'}>
                        <td className="px-4 py-2 text-center text-sm font-mono" style={{ color: 'rgba(255, 255, 255, 0.9)' }}>{response.sessionId}</td>
                        <td className="px-4 py-2 text-center text-sm font-medium" style={{ color: 'rgba(255, 255, 255, 0.9)' }}>{response.service}</td>
                        <td className="px-4 py-2 text-center text-sm font-semibold" style={{ color: 'rgba(59, 130, 246, 1)' }}>{response.count}</td>
                        <td className="px-4 py-2 text-center text-sm" style={avgStyle}>{response.avg}</td>
                        <td className="px-4 py-2 text-center text-sm" style={{ color: 'rgba(255, 255, 255, 0.8)' }}>{response.modelMs}</td>
                        <td className="px-4 py-2 text-center text-sm" style={{ color: 'rgba(255, 255, 255, 0.8)' }}>{response.networkMs}</td>
                        <td className="px-4 py-2 text-center text-sm font-semibold" style={totalStyle}>{response.totalMs}</td>
                        <td className="px-4 py-2 text-center text-sm font-medium" style={{ color: 'rgba(252, 165, 165, 1)' }}>{response.max}</td>
                        <td className="px-4 py-2 text-center text-sm font-medium" style={{ color: 'rgba(134, 239, 172, 1)' }}>{response.min}</td>
                        <td className="px-4 py-2 text-center text-sm" style={{ color: 'rgba(255, 255, 255, 0.8)' }}>{formatDateTime(response.lastTime)}</td>
                      </tr>
                    );
                  })}
                </tbody>

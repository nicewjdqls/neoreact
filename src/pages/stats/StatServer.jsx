...
function StatServer() {
  ...
  const addResource = () => {
    ...
    const newResource = {
      id: uniqId(),
      server,
      gpuUsage: parseFloat(gpuUsage),
      gpuMem: parseInt(gpuMem),
    ...
    try {
      ...
      if (filters.serverFilter) {
        ...
        filtered = filtered.filter(resource => 
          resource.server.toLowerCase().includes(serverLower)
        );
      }
      ...
      setTimeout(() => {
        showNotification(
          '조회 완료',
          `총 ${filtered.length}건의 서버 자원 데이터가 조회되었습니다.`,
          'success'
        );
      }, 100);
  ...
  const getUsageClass = (value, type) => {
    ...
    if (value > threshold1) return 'text-orange-600 font-semibold';
    ...
    >
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
          <h2 className="text-xl font-bold text-gray-900">GPU / 서버 자원 상세조회</h2>
        </div>

        {/* 필터 영역 */}
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
          {/* 첫 번째 행: 시작일시 - 종료일시 - 서버 - 조회버튼 */}
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end mb-4">
            ...
            {/* 서버 필터 */}
            <div className="md:col-span-1">
              <label className="block text-sm font-medium text-gray-600 mb-1">서버</label>
              <input
                type="text"
                placeholder="서버명 검색..."
                value={filters.serverFilter}
                onChange={(e) => handleFilterChange('serverFilter', e.target.value)}
        ...
        <div className="flex justify-between items-center">
          <span className="text-sm text-gray-600">
            총 {filteredResources.length}건의 서버 자원 데이터가 조회되었습니다.
          </span>
          {autoScroll && (
            ...
            {filteredResources.length > 0 ? (
              <table className="w-full">
                <thead className="bg-gray-50 sticky top-0 z-10">
                  <tr>
                    <th className="px-4 py-3 text-center text-sm font-semibold text-gray-600 border-b border-gray-200">시간</th>
                    <th className="px-4 py-3 text-center text-sm font-semibold text-gray-600 border-b border-gray-200">서버</th>
                    <th className="px-4 py-3 text-center text-sm font-semibold text-gray-600 border-b border-gray-200">GPU 사용률(%)</th>
                    <th className="px-4 py-3 text-center text-sm font-semibold text-gray-600 border-b border-gray-200">GPU 메모리 사용량(MB)</th>
                    <th className="px-4 py-3 text-center text-sm font-semibold text-gray-600 border-b border-gray-200">CPU 사용률(%)</th>
                    <th className="px-4 py-3 text-center text-sm font-semibold text-gray-600 border-b border-gray-200">메모리 사용량(GB)</th>
                    <th className="px-4 py-3 text-center text-sm font-semibold text-gray-600 border-b border-gray-200">GPU 온도(℃)</th>
                    <th className="px-4 py-3 text-center text-sm font-semibold text-gray-600 border-b border-gray-200">프로세스 수</th>
                    <th className="px-4 py-3 text-center text-sm font-semibold text-gray-600 border-b border-gray-200">큐 대기 수</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-200">
                  {filteredResources.map(resource => (
                    <tr key={resource.id} className="hover:bg-gray-50">
                      <td className="px-4 py-2 text-center text-sm text-gray-700">{formatDateTime(resource.timestamp)}</td>
                      <td className="px-4 py-2 text-center text-sm font-medium text-gray-900">{resource.server}</td>
                      <td className={`px-4 py-2 text-center text-sm ${getUsageClass(resource.gpuUsage, 'gpu')}`}>
                        {resource.gpuUsage}%
                      </td>
                      <td className="px-4 py-2 text-center text-sm text-gray-700">{resource.gpuMem.toLocaleString()}</td>
                      <td className={`px-4 py-2 text-center text-sm ${getUsageClass(resource.cpuUsage, 'cpu')}`}>
                        {resource.cpuUsage}%
                      </td>
                      <td className={`px-4 py-2 text-center text-sm ${getUsageClass(resource.memUsage, 'mem')}`}>
                        {resource.memUsage}
                      </td>
                      <td className={`px-4 py-2 text-center text-sm ${getUsageClass(resource.temp, 'temp')}`}>
                        {resource.temp}℃
                      </td>
                      <td className="px-4 py-2 text-center text-sm text-gray-700">{resource.processCount}</td>

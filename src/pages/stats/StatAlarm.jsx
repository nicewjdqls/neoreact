...
function StatAlarm() {
  ...
  const addEvent = () => {
    ...
    if (selectedLevel === 'DELAY') {
      ...
      selectedMessage = delayMessages.length > 0 
  ...
  const applyFilters = async () => {
    ...
    try {
      ...
      if (filters.startDate) {
        ...
        filtered = filtered.filter(event => event.rawTime >= startTime);
      }
      ...
      if (filters.endDate) {
        ...
        filtered = filtered.filter(event => event.rawTime <= endOfDay);
      }

      if (filters.filterLevel) {
        filtered = filtered.filter(event => event.level === filters.filterLevel);
      }
      ...
      if (filters.filterText) {
        ...
        filtered = filtered.filter(event => 
          ...
          event.node.toLowerCase().includes(searchLower) ||
      ...
      setTimeout(() => {
        showNotification(
          '조회 완료',
          `총 ${filtered.length}건의 알림 이벤트가 조회되었습니다.`,
          'success'
        );
      }, 100);
  ...
  const getLevelBadgeStyle = (level) => {
    switch (level) {
      case 'INFO': return { background: 'rgba(59, 130, 246, 0.2)', color: '#60a5fa' };
      case 'WARN': return { background: 'rgba(245, 158, 11, 0.2)', color: '#fbbf24' };
      case 'ERROR': return { background: 'rgba(239, 68, 68, 0.2)', color: '#f87171' };
      case 'DELAY': return { background: 'rgba(6, 182, 212, 0.2)', color: '#22d3ee' };
      ...
      <div style={{ padding: '2rem', maxWidth: '1600px', margin: '0 auto' }}>
        <div style={{ marginBottom: '1.5rem' }}>
          ...
          <div style={{
            ...
            <h2 style={{ fontSize: '1.5rem', fontWeight: '700', color: '#fff', margin: 0 }}>알림 & 이벤트 상세조회</h2>
          </div>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', gap: '1rem', alignItems: 'end', marginBottom: '1rem', flexWrap: 'wrap' }}>
              ...
              <div style={{ flex: '0 0 auto' }}>
                <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer', paddingTop: '2rem' }}>
                  <input
                    type="checkbox"
                    checked={autoScroll}
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
            <span style={{ fontSize: '0.875rem', color: 'rgba(255, 255, 255, 0.7)' }}>
              총 {filteredEvents.length}건의 알림 이벤트가 조회되었습니다.
            </span>
            {autoScroll && (
              <div style={{
                display: 'flex',
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead style={{ position: 'sticky', top: 0, zIndex: 10 }}>
                    <tr style={{ background: 'rgba(30, 33, 57, 0.95)' }}>
                      <th style={{ padding: '1rem', textAlign: 'center', fontSize: '0.875rem', fontWeight: '600', color: 'rgba(255, 255, 255, 0.7)', borderBottom: '1px solid rgba(99, 102, 241, 0.2)' }}>발생일시</th>
                      <th style={{ padding: '1rem', textAlign: 'center', fontSize: '0.875rem', fontWeight: '600', color: 'rgba(255, 255, 255, 0.7)', borderBottom: '1px solid rgba(99, 102, 241, 0.2)' }}>Level</th>
                      <th style={{ padding: '1rem', textAlign: 'center', fontSize: '0.875rem', fontWeight: '600', color: 'rgba(255, 255, 255, 0.7)', borderBottom: '1px solid rgba(99, 102, 241, 0.2)' }}>이벤트 메시지</th>
                      <th style={{ padding: '1rem', textAlign: 'center', fontSize: '0.875rem', fontWeight: '600', color: 'rgba(255, 255, 255, 0.7)', borderBottom: '1px solid rgba(99, 102, 241, 0.2)' }}>대상 노드</th>
                    ...
                    {filteredEvents.map((event, index) => (
                      <tr 
                        ...
                        style={{
                          ...
                          background: (autoScroll && index === 0) ? 'rgba(99, 102, 241, 0.15)' : 'transparent'
                        }}
                        onMouseEnter={(e) => e.currentTarget.style.background = 'rgba(99, 102, 241, 0.1)'}
                        onMouseLeave={(e) => e.currentTarget.style.background = (autoScroll && index === 0) ? 'rgba(99, 102, 241, 0.15)' : 'transparent'}
                      >
                        <td style={{ padding: '0.75rem', textAlign: 'center', fontSize: '0.875rem', color: 'rgba(255, 255, 255, 0.8)' }}>{event.timestamp}</td>
                        <td style={{ padding: '0.75rem', textAlign: 'center' }}>
                          <span style={{
                            ...
                            fontWeight: '600',
                            borderRadius: '9999px',
                            ...getLevelBadgeStyle(event.level)
                          }}>
                            {event.level}
                          </span>
                        </td>
                        <td style={{ padding: '0.75rem', fontSize: '0.875rem', color: 'rgba(255, 255, 255, 0.8)' }}>{event.message}</td>
                        <td style={{ padding: '0.75rem', textAlign: 'center', fontSize: '0.875rem', fontFamily: 'monospace', color: 'rgba(255, 255, 255, 0.8)' }}>{event.node}</td>
                        <td style={{ padding: '0.75rem', textAlign: 'center', fontSize: '0.875rem', color: 'rgba(255, 255, 255, 0.8)' }}>{event.service}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>

...
function StatAlarm() {
  ...
  // 실시간 이벤트 생성
  useEffect(() => {
    const interval = setInterval(addEvent, 2000); // 2초마다 생성
    ...
    >
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
          <h2 className="text-xl font-bold text-gray-900">알림 & 이벤트 상세조회</h2>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
          <div className="grid grid-cols-1 md:grid-cols-6 gap-4 items-end mb-4">
            ...
            {/* 검색 */}
            <div className="md:col-span-1">
              <label className="block text-sm font-medium text-gray-600 mb-1">검색</label>
              <input
                type="text"
            ...
            <div className="md:col-span-1">
              <button
                ...
                className="w-full min-w-[120px] h-12 px-6 py-3 text-sm font-medium text-white bg-gradient-to-r from-violet-500 to-purple-600 rounded-xl hover:from-violet-600 hover:to-purple-700 shadow-sm transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center whitespace-nowrap"
              >
                {isLoading ? (
            ...
            {filteredEvents.length > 0 ? (
              <table className="w-full">
                <thead className="bg-gray-50 sticky top-0 z-10">
                  <tr>
                    <th className="px-4 py-3 text-center text-sm font-semibold text-gray-600 border-b border-gray-200">발생일시</th>
                    <th className="px-4 py-3 text-center text-sm font-semibold text-gray-600 border-b border-gray-200">Level</th>
                    <th className="px-4 py-3 text-center text-sm font-semibold text-gray-600 border-b border-gray-200">이벤트 메시지</th>
                    <th className="px-4 py-3 text-center text-sm font-semibold text-gray-600 border-b border-gray-200">대상 노드</th>
                    <th className="px-4 py-3 text-center text-sm font-semibold text-gray-600 border-b border-gray-200">서비스 구분</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-200">
                  {filteredEvents.map((event, index) => (
                    <tr key={event.id} className={`hover:bg-gray-50 ${autoScroll && index === 0 ? 'bg-blue-50' : ''}`}>
                      <td className="px-4 py-2 text-center text-sm text-gray-700">{event.timestamp}</td>
                      <td className="px-4 py-2 text-center">
                        <span className={`inline-block px-2 py-1 text-xs font-medium rounded-full ${getLevelBadgeClass(event.level)}`}>
                          {event.level}
                        </span>
                      </td>
                      <td className="px-4 py-2 text-sm text-gray-700">{event.message}</td>
                      <td className="px-4 py-2 text-center text-sm text-gray-700 font-mono">{event.node}</td>
                      <td className="px-4 py-2 text-center text-sm text-gray-700">{event.service}</td>
                    </tr>
                  ))}
                </tbody>
              </table>

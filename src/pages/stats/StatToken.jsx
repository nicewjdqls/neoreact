...
function StatToken() {
  ...
  const getTokenClass = (count) => {
    if (count > 1000) return 'text-red-600 font-semibold';
    if (count > 500) return 'text-orange-600 font-semibold';
  ...
  const getModelBadgeClass = (model) => {
    switch (model) {
      ...
      case 'gpt-3.5': return 'bg-blue-100 text-blue-800';
    ...
    >
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
          <h2 className="text-xl font-bold text-gray-900">토큰 사용량 상세조회</h2>
        </div>

        {/* 필터 영역 */}
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
          {/* 첫 번째 행: 시작일시 - 종료일시 - 서비스구분 - 모델 - 조회버튼 */}
              ...
              <table className="w-full">
                <thead className="bg-gray-50 sticky top-0 z-10">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-center text-sm font-semibold text-gray-600 border-b border-gray-200">총 토큰</th>
                    <th className="px-4 py-3 text-center text-sm font-semibold text-gray-600 border-b border-gray-200">입력 토큰</th>
                    <th className="px-4 py-3 text-center text-sm font-semibold text-gray-600 border-b border-gray-200">출력 토큰</th>
                ...
                <tbody className="divide-y divide-gray-200">
                  {filteredTokens.map(token => (
                    <tr key={token.id} className="hover:bg-gray-50">
                      <td className="px-4 py-2 text-center text-sm font-mono text-gray-900">{token.sessionId}</td>
                      <td className="px-4 py-2 text-center text-sm font-medium text-gray-900">{token.service}</td>
                      <td className={`px-4 py-2 text-center text-sm ${getTokenClass(token.total)}`}>
                        {token.total.toLocaleString()}
                      </td>
                      <td className="px-4 py-2 text-center text-sm text-gray-700">{token.input.toLocaleString()}</td>
                      <td className="px-4 py-2 text-center text-sm text-gray-700">{token.output.toLocaleString()}</td>
                      <td className="px-4 py-2 text-center">
                        <span className={`inline-block px-2 py-1 text-xs font-medium rounded-full ${getModelBadgeClass(token.model)}`}>
                          {token.model}
                        </span>
                      </td>
                      <td className="px-4 py-2 text-center text-sm text-gray-700">{formatDateTime(token.lastTime)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>

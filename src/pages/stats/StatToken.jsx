...
function StatToken() {
  ...
  const addTokenUsage = () => {
    ...
    const input = Math.floor(Math.random() * 500) + 50;
    const output = Math.floor(Math.random() * 500) + 50;
  ...
  const applyFilters = async () => {
    ...
    try {
      ...
      if (filters.startDate) {
        ...
        filtered = filtered.filter(token => token.rawTime >= startTime);
      }
      ...
      if (filters.endDate) {
        ...
        filtered = filtered.filter(token => token.rawTime <= endOfDay);
      }

      if (filters.serviceType) {
        filtered = filtered.filter(token => token.service === filters.serviceType);
      }

      if (filters.modelFilter) {
        filtered = filtered.filter(token => token.model === filters.modelFilter);
      }
      ...
      setTimeout(() => {
        showNotification(
          '조회 완료',
          `총 ${filtered.length}건의 토큰 사용량 데이터가 조회되었습니다.`,
          'success'
        );
      }, 100);
  ...
  const getModelBadgeStyle = (model) => {
    switch (model) {
      case 'gpt-4': return { background: 'rgba(168, 85, 247, 0.2)', color: '#c084fc' };
      case 'gpt-3.5': return { background: 'rgba(99, 102, 241, 0.2)', color: '#a5b4fc' };
  ...
  useEffect(() => {
    const interval = setInterval(addTokenUsage, 1500);
    return () => clearInterval(interval);
  }, []);

  useEffect(() => {
    if (autoScroll && tokens.length > 0) {
      ...
      <div style={{ padding: '2rem', maxWidth: '1600px', margin: '0 auto' }}>
        <div style={{ marginBottom: '1.5rem' }}>
          ...
          <div style={{
            ...
            <h2 style={{ fontSize: '1.5rem', fontWeight: '700', color: '#fff', margin: 0 }}>토큰 사용량 상세조회</h2>
          </div>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', gap: '1rem', alignItems: 'end', marginBottom: '1rem' }}>
              ...
              {/* 모델 */}
              <div style={{ flex: 1 }}>
                <label style={{ display: 'block', fontSize: '0.875rem', fontWeight: '600', color: 'rgba(255, 255, 255, 0.7)', marginBottom: '0.5rem' }}>모델</label>
                <select
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
            <span style={{ fontSize: '0.875rem', color: 'rgba(255, 255, 255, 0.7)' }}>
              총 {filteredTokens.length}건의 토큰 사용량 데이터가 조회되었습니다.
            </span>
            {autoScroll && (
              <div style={{
                display: 'flex',
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead style={{ position: 'sticky', top: 0, zIndex: 10 }}>
                    <tr style={{ background: 'rgba(30, 33, 57, 0.95)' }}>
                      <th style={{ padding: '1rem', textAlign: 'center', fontSize: '0.875rem', fontWeight: '600', color: 'rgba(255, 255, 255, 0.7)', borderBottom: '1px solid rgba(99, 102, 241, 0.2)' }}>세션ID</th>
                      <th style={{ padding: '1rem', textAlign: 'center', fontSize: '0.875rem', fontWeight: '600', color: 'rgba(255, 255, 255, 0.7)', borderBottom: '1px solid rgba(99, 102, 241, 0.2)' }}>서비스 구분</th>
                      <th style={{ padding: '1rem', textAlign: 'center', fontSize: '0.875rem', fontWeight: '600', color: 'rgba(255, 255, 255, 0.7)', borderBottom: '1px solid rgba(99, 102, 241, 0.2)' }}>총 토큰</th>
                      <th style={{ padding: '1rem', textAlign: 'center', fontSize: '0.875rem', fontWeight: '600', color: 'rgba(255, 255, 255, 0.7)', borderBottom: '1px solid rgba(99, 102, 241, 0.2)' }}>입력 토큰</th>
                      <th style={{ padding: '1rem', textAlign: 'center', fontSize: '0.875rem', fontWeight: '600', color: 'rgba(255, 255, 255, 0.7)', borderBottom: '1px solid rgba(99, 102, 241, 0.2)' }}>출력 토큰</th>
                      ...
                      <th style={{ padding: '1rem', textAlign: 'center', fontSize: '0.875rem', fontWeight: '600', color: 'rgba(255, 255, 255, 0.7)', borderBottom: '1px solid rgba(99, 102, 241, 0.2)' }}>마지막 요청 시간</th>
                    </tr>
                  </thead>
                  <tbody>
                    {filteredTokens.map(token => (
                      <tr 
                        ...
                        onMouseLeave={(e) => e.currentTarget.style.background = 'transparent'}
                      >
                        <td style={{ padding: '0.75rem', textAlign: 'center', fontSize: '0.875rem', fontFamily: 'monospace', color: 'rgba(255, 255, 255, 0.9)' }}>{token.sessionId}</td>
                        <td style={{ padding: '0.75rem', textAlign: 'center', fontSize: '0.875rem', fontWeight: '600', color: 'rgba(255, 255, 255, 0.9)' }}>{token.service}</td>
                        <td style={{ padding: '0.75rem', textAlign: 'center', fontSize: '0.875rem', fontWeight: '600', color: getTokenClass(token.total) }}>
                          {token.total.toLocaleString()}
                        </td>
                        <td style={{ padding: '0.75rem', textAlign: 'center', fontSize: '0.875rem', color: 'rgba(255, 255, 255, 0.8)' }}>{token.input.toLocaleString()}</td>
                        <td style={{ padding: '0.75rem', textAlign: 'center', fontSize: '0.875rem', color: 'rgba(255, 255, 255, 0.8)' }}>{token.output.toLocaleString()}</td>
                        <td style={{ padding: '0.75rem', textAlign: 'center' }}>
                          <span style={{
                            ...
                            fontWeight: '600',
                            borderRadius: '9999px',
                            ...getModelBadgeStyle(token.model)
                          }}>
                            {token.model}
                          </span>
                        </td>
                        <td style={{ padding: '0.75rem', textAlign: 'center', fontSize: '0.875rem', color: 'rgba(255, 255, 255, 0.8)' }}>{formatDateTime(token.lastTime)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>

...
function StatNode() {
  ...
  const getUsageClass = (value) => {
    if (value > 85) return { color: 'rgba(252, 165, 165, 1)', fontWeight: '600' };
    if (value > 60) return { color: 'rgba(251, 146, 60, 1)', fontWeight: '600' };
        ...
        <div style={{ 
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4 items-end mb-4">
            <div className="md:col-span-1">
              <label className="block text-sm font-medium mb-1 text-left" style={{ color: 'rgba(255, 255, 255, 0.8)' }}>시작일시</label>
            ...
            <div className="md:col-span-1">
              <label className="block text-sm font-medium mb-1 text-left" style={{ color: 'rgba(255, 255, 255, 0.8)' }}>종료일시</label>
              <CustomDateTimePicker
            ...
            <div className="md:col-span-1">
              <label className="block text-sm font-medium mb-1" style={{ color: 'rgba(255, 255, 255, 0.8)' }}>노드명 검색</label>
              <input
                type="text"
                placeholder="Search..."
            ...
            <div className="md:col-span-1">
              <button
                ...
                className="w-full h-12 px-6 py-3 text-sm font-medium rounded-xl transition-all duration-200 flex items-center justify-center whitespace-nowrap"
                style={{
                  color: '#fff',
                  background: (isLoading || !isDateTimeRangeValid())
                    ? 'rgba(139, 92, 246, 0.5)' 
                    : 'linear-gradient(135deg, rgba(109, 40, 217, 0.9), rgba(139, 92, 246, 0.9))',
              ...
              <table className="w-full">
                ...
                <tbody>
                  {filteredNodes.map(node => {
                    const statusStyle = getStatusBadgeClass(node.status);
                    const cpuStyle = getUsageClass(node.cpu);
                    const memStyle = getUsageClass(node.mem);
                    const diskStyle = getUsageClass(node.disk);
                    return (
                      <tr key={node.id} className="border-b transition-colors" style={{ 
                        borderColor: 'rgba(139, 92, 246, 0.1)'
                      }}
                      onMouseEnter={(e) => e.currentTarget.style.background = 'rgba(139, 92, 246, 0.05)'}
                      onMouseLeave={(e) => e.currentTarget.style.background = 'transparent'}
                      >
                        <td className="px-4 py-2 text-center text-sm font-semibold" style={{ color: 'rgba(255, 255, 255, 0.9)' }}>{node.name}</td>
                        <td className="px-4 py-2 text-center text-sm font-mono" style={{ color: 'rgba(255, 255, 255, 0.8)' }}>{node.ip}</td>
                        <td className="px-4 py-2 text-center text-sm" style={{ color: 'rgba(255, 255, 255, 0.8)' }}>{node.host}</td>
                        <td className="px-4 py-2 text-center">
                          <span className="inline-block px-2 py-1 text-xs font-medium rounded-full" style={statusStyle}>
                            {node.status}
                          </span>
                        </td>
                        <td className="px-4 py-2 text-center text-sm" style={cpuStyle}>
                          {node.cpu}%
                        </td>
                        <td className="px-4 py-2 text-center text-sm" style={memStyle}>
                          {node.mem}%
                        </td>
                        <td className="px-4 py-2 text-center text-sm" style={diskStyle}>
                          {node.disk}%
                        </td>
                        <td className="px-4 py-2 text-center text-sm" style={{ color: 'rgba(255, 255, 255, 0.8)' }}>{node.net}</td>
                        <td className="px-4 py-2 text-center text-sm" style={{ color: 'rgba(255, 255, 255, 0.8)' }}>{node.uptime}</td>
                        <td className="px-4 py-2 text-center text-sm" style={{ color: 'rgba(255, 255, 255, 0.8)' }}>{node.lastCheck}</td>
                      </tr>
                    );
                  })}
                </tbody>

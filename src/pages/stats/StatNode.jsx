...
function StatNode() {
  ...
  const hosts = ['node01', 'node02', 'node03', 'node04', 'node05'];
  ...
  const addNode = () => {
    const now = new Date();
    const name = hosts[Math.floor(Math.random() * hosts.length)];
    const ip = `192.168.0.${Math.floor(Math.random() * 255)}`;
    const host = name + '.local';
    const status = ['OK', 'WARN', 'ERROR'][Math.floor(Math.random() * 3)];
    const cpu = Math.floor(Math.random() * 100);
    const mem = Math.floor(Math.random() * 100);
    const disk = Math.floor(Math.random() * 100);
    const net = (Math.random() * 100).toFixed(2);
    const uptime = `${Math.floor(Math.random() * 30)}일 ${Math.floor(Math.random() * 24)}시간`;
    const lastCheck = formatDateTime(now);
  ...
  const applyFilters = async () => {
    ...
    try {
      ...
      if (filters.startDate) {
        ...
        filtered = filtered.filter(node => node.rawTime >= startTime);
      }
      ...
      if (filters.endDate) {
        ...
        filtered = filtered.filter(node => node.rawTime <= endOfDay);
      }

      if (filters.filterStatus) {
        filtered = filtered.filter(node => node.status === filters.filterStatus);
      }
      ...
      setTimeout(() => {
        showNotification(
          '조회 완료',
          `총 ${filtered.length}건의 노드 상태 데이터가 조회되었습니다.`,
          'success'
        );
      }, 100);
  ...
  const getUsageClass = (value) => {
    if (value > 85) return 'text-red-600 font-semibold';
    if (value > 60) return 'text-orange-600 font-semibold';
  ...
  // 실시간 노드 상태 데이터 생성
  useEffect(() => {
    const interval = setInterval(addNode, 2000);
    return () => clearInterval(interval);
  }, []);

  // 자동 스크롤이 켜져있으면 필터 자동 적용
  useEffect(() => {
    if (autoScroll && nodes.length > 0) {
    ...
    >
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
          <h2 className="text-xl font-bold text-gray-900">노드 상태 상세조회</h2>
        </div>

        {/* 필터 영역 */}
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
          {/* 한 줄 레이아웃: 시작일시 - 종료일시 - 상태 - 노드명검색 - 조회버튼 */}
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4 items-end mb-4">
            {/* 시작일시 - 커스텀 DateTime 피커 */}
            <div className="md:col-span-1">
              <label className="block text-sm font-medium text-gray-600 mb-1 text-left">시작일시</label>
              <CustomDateTimePicker
                value={filters.startDate}
                onChange={(value) => handleFilterChange('startDate', value)}
                placeholder="시작일시 선택"
                className={!isDateTimeRangeValid() ? 'border-red-300 text-left' : 'text-left'}
              />
            </div>

            {/* 종료일시 - 커스텀 DateTime 피커 */}
            <div className="md:col-span-1">
              <label className="block text-sm font-medium text-gray-600 mb-1 text-left">종료일시</label>
              <CustomDateTimePicker
            ...
            {/* 노드명 검색 */}
            <div className="md:col-span-1">
              <label className="block text-sm font-medium text-gray-600 mb-1">노드명 검색</label>
              <input
                type="text"
                placeholder="Search..."
            ...
            <div className="md:col-span-1">
              <button
                ...
                className="w-full h-12 px-6 py-3 text-sm font-medium text-white bg-gradient-to-r from-violet-500 to-purple-600 rounded-xl hover:from-violet-600 hover:to-purple-700 shadow-sm transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center whitespace-nowrap"
              >
                {isLoading ? (
        ...
        <div className="flex justify-between items-center">
          <span className="text-sm text-gray-600">
            총 {filteredNodes.length}건의 노드 상태 데이터가 조회되었습니다.
          </span>
          {autoScroll && (
            ...
            {filteredNodes.length > 0 ? (
              <table className="w-full">
                <thead className="bg-gray-50 sticky top-0 z-10">
                  <tr>
                    <th className="px-4 py-3 text-center text-sm font-semibold text-gray-600 border-b border-gray-200">노드명</th>
                    <th className="px-4 py-3 text-center text-sm font-semibold text-gray-600 border-b border-gray-200">IP</th>
                    <th className="px-4 py-3 text-center text-sm font-semibold text-gray-600 border-b border-gray-200">호스트명</th>
                    <th className="px-4 py-3 text-center text-sm font-semibold text-gray-600 border-b border-gray-200">상태</th>
                    <th className="px-4 py-3 text-center text-sm font-semibold text-gray-600 border-b border-gray-200">CPU 사용률(%)</th>
                    <th className="px-4 py-3 text-center text-sm font-semibold text-gray-600 border-b border-gray-200">메모리 사용률(%)</th>
                    <th className="px-4 py-3 text-center text-sm font-semibold text-gray-600 border-b border-gray-200">디스크 사용률(%)</th>
                    <th className="px-4 py-3 text-center text-sm font-semibold text-gray-600 border-b border-gray-200">네트워크(Mbps)</th>
                    <th className="px-4 py-3 text-center text-sm font-semibold text-gray-600 border-b border-gray-200">업타임</th>
                    <th className="px-4 py-3 text-center text-sm font-semibold text-gray-600 border-b border-gray-200">마지막 점검일시</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-200">
                  {filteredNodes.map(node => (
                    <tr key={node.id} className="hover:bg-gray-50">
                      <td className="px-4 py-2 text-center text-sm font-semibold text-gray-900">{node.name}</td>
                      <td className="px-4 py-2 text-center text-sm font-mono text-gray-700">{node.ip}</td>
                      <td className="px-4 py-2 text-center text-sm text-gray-700">{node.host}</td>
                      <td className="px-4 py-2 text-center">
                        <span className={`inline-block px-2 py-1 text-xs font-medium rounded-full ${getStatusBadgeClass(node.status)}`}>
                          {node.status}
                        </span>
                      </td>
                      <td className={`px-4 py-2 text-center text-sm ${getUsageClass(node.cpu)}`}>
                        {node.cpu}%
                      </td>
                      <td className={`px-4 py-2 text-center text-sm ${getUsageClass(node.mem)}`}>
                        {node.mem}%
                      </td>
                      <td className={`px-4 py-2 text-center text-sm ${getUsageClass(node.disk)}`}>
                        {node.disk}%
                      </td>
                      <td className="px-4 py-2 text-center text-sm text-gray-700">{node.net}</td>
                      <td className="px-4 py-2 text-center text-sm text-gray-700">{node.uptime}</td>
                      <td className="px-4 py-2 text-center text-sm text-gray-700">{node.lastCheck}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
